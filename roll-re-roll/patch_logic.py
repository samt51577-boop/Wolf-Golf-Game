...
new_lines = []
in_roll_block = False
roll_block_replaced = False

# New Logic Block
high_low_logic = """        if (this.state.gameMode === 'roll') {
            ...
            let amount = matchBet; // Default amount for standard play tracking
...
for line in lines:
    if "if (this.state.gameMode === 'roll') {" in line:
        in_roll_block = True
        if not roll_block_replaced:
            new_lines.append(high_low_logic)
            roll_block_replaced = True
    
    if in_roll_block:
        if "} else if (this.state.gameMode === 'umbrella') {" in line:
            in_roll_block = False
            new_lines.append(line) # Add the else if line back
    else:
